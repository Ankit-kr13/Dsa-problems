...
int main()
{
    char string[100], word[100], remove_word[100], new_string[100] = "\0";
    int i, j, len;
    ...
    for (i = 0; string[i]; i++)
    {
        if (string[i] == ' ')
        {
            substring(string, word, j, i);
            j = i+1;
            if(strcmp(word,remove_word)!=0)
            {
            concatenate(word, new_string);
            }
        }
    }
    printf("the string after removing the desired word is : %s",new_string);
    return 0;
}